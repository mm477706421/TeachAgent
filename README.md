...
- **清晰的对话阅读**：新回答逐字显示，可立即展开全文；Markdown 标题、列表、引用、代码块与表格正确排版，历史回答直接完整展示。
...
登录后进入 **空间设置 → 模型服务设置**，选择“OpenAI 兼容服务”，填写 Base URL（例如 `https://api.openai.com/v1`）、服务支持的模型 ID、API Key，确认文本发送范围，点击“测试连接”，成功后点击“保存模型设置”。测试使用当前表单，不自动保存、不发送课堂材料。Base URL 支持 HTTP 和 HTTPS（例如 `http://192.168.1.10:8000/v1`）。支持非流式 `/chat/completions`，参数包括超时、温度与输出 token 上限；服务需支持这些参数。
